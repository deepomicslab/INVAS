...
Vertex *Graph::addVertex(std::string mId, std::string aChrom, int aStart, int aEnd,float aCoverage, float mCredibility, int aCopyNum, int copy_idx, float cov, std::string signal, float conCoverage) {
//    create vertex add push
    auto v1 = this->getVertexById(mId);
    if(v1 != nullptr) return v1;
    auto *vertex = new Vertex(mId, aChrom, aStart, aEnd, aCoverage, mCredibility, aCopyNum, this->vertices->size(),copy_idx, cov);
    if(signal=="INV"){
        vertex->setSignal("INV");
        vertex->getWeight()->setConCoverage(conCoverage);
    }
    //    vertex->setIdx(this->vertices->size());
    this->vertices->push_back(vertex);
...
Junction *
Graph::addSourceSinkJunction(Vertex *sourceVertex, Vertex *targetVertex, char sourceDir, char targetDir, float copyNum,
                             float coverage, bool aIsBounded) {
...
float ** Graph::getConjugateMatrix2() {
    return ConjugateMatrix;
}

std::map<std::pair<int, int>, std::pair<int, int>> &Graph::getConjugateIndexMap() {
    return ConjugateIndexMap;
}

void Graph::initMatrix(){
    int N = this->vertices->size();
    ConjugateMatrix = new float*[4*N-2];
    for (int i = 0; i < 4*N-2; ++i) {
        ConjugateMatrix[i] = new float[4*N-2];
    }
    for (int i = 0; i < 4*N-2; ++i) {
        for (int j = 0; j < 4*N-2; ++j) {
            ConjugateMatrix[i][j] = 0.0f;
        }
    }
    /// get adjacent matrix in loop
    for (auto v : *this->vertices) {
        int node_id = v->getIdx();
        int s_matrix_row, s_matrix_col, t_matrix_row, t_matrix_col;
        int con_s_matrix_row, con_s_matrix_col, con_t_matrix_row, con_t_matrix_col;

        /// for source node
        if (v->getIsGSource()){
            /// add 5'-3' edge
            int s_matrix_row = s_matrix_col = 4*node_id;
            int t_matrix_row = t_matrix_col = 4*node_id+1;
            ConjugateMatrix[s_matrix_row][t_matrix_col] = INF;
            /// get ep3
            auto ep3_edges = v->getEp3()->getEdges();
            for (auto e : *ep3_edges) {
                int target_node_id = e->getTarget()->getVertex()->getIdx();
                s_matrix_row = s_matrix_col = 4*node_id+1;
                if (e->getTarget()->getType()==5){
                    t_matrix_row = t_matrix_col = 4*target_node_id;
                }else if(e->getTarget()->getType()==-5){
                    t_matrix_row = t_matrix_col = 4*target_node_id+2;
                }
                /// add val to matrix
                ConjugateMatrix[s_matrix_row][t_matrix_col] = INF;
            }
        ...
        if (v->getIsGSink()){
            /// get ep5
            auto ep5_edges = v->getEp5()->getEdges();
            for (auto e : *ep5_edges) {
                int source_node_id = e->getSource()->getVertex()->getIdx();
                if (e->getSource()->getType()==3){
                    s_matrix_row = s_matrix_col = 4*source_node_id+1;
                } else if (e->getSource()->getType()==-3){
                    s_matrix_row = s_matrix_col = 4*source_node_id+3;
                }

                t_matrix_row = t_matrix_col = 4*node_id-2;
                ConjugateMatrix[s_matrix_row][t_matrix_col] = INF;
            }

            /// add 5'-3'
            s_matrix_row = s_matrix_col = 4*node_id-2;
            t_matrix_row = t_matrix_col = 4*node_id-1;
            ConjugateMatrix[s_matrix_row][t_matrix_col] = INF;
            continue;
        }
        /// get normal edges
        ...
        auto ep3_edges = v->getEp3()->getEdges();
        for (auto e : *ep3_edges) {
            if (e->getTarget()->getVertex()->getIsGSink()) continue;
            int target_node_id = e->getTarget()->getVertex()->getIdx();
            s_matrix_row = s_matrix_col = 4*node_id+1;
            con_t_matrix_col = s_matrix_row + 1;
            if (e->getTarget()->getType()==5){
                t_matrix_row = t_matrix_col = 4*target_node_id;
                con_s_matrix_row = t_matrix_row + 3;
            }else if(e->getTarget()->getType()==-5){
                t_matrix_row = t_matrix_col = 4*target_node_id+2;
                con_s_matrix_row = t_matrix_row - 1;
            }
            /// add val to matrix
            ConjugateMatrix[s_matrix_row][t_matrix_col] = e->getWeight()->getCapacity();
//            ConjugateMatrix[con_s_matrix_row][con_t_matrix_col] = e->getWeight()->getCapacity();
            ConjugateIndexMap[std::make_pair(s_matrix_row, t_matrix_col)] = std::make_pair(con_s_matrix_row, con_t_matrix_col);
            ConjugateIndexMap[std::make_pair(con_s_matrix_row, con_t_matrix_col)] = std::make_pair(s_matrix_row, t_matrix_col);
//            std::map<std::pair<int, int>, std::pair<int, int>> ConjugateIndexMap;

        }
        /// for neg ep3
        auto rep3_edges = v->getRep3()->getEdges();
        for (auto e: *rep3_edges){
            if (e->getTarget()->getVertex()->getIsGSink()) continue;
            int target_node_id = e->getTarget()->getVertex()->getIdx();
            s_matrix_row = s_matrix_col = 4*node_id+3;
            con_t_matrix_col = s_matrix_row - 3;
            if (e->getTarget()->getType()==5){
                t_matrix_row = t_matrix_col = 4*target_node_id;
                con_s_matrix_row = t_matrix_row + 3;
            }else if(e->getTarget()->getType()==-5){
...
//            ConjugateMatrix[con_s_matrix_row][con_t_matrix_col] = e->getWeight()->getCapacity();
        ...
        /// add 5'-3' inner edge
        s_matrix_row = s_matrix_col = 4*node_id;
        t_matrix_row = t_matrix_col = 4*node_id+1;
        ConjugateMatrix[s_matrix_row][t_matrix_col] = v->getWeight()->getCapacity();
        ConjugateIndexMap[std::make_pair(s_matrix_row, t_matrix_col)] = std::make_pair(s_matrix_row+2, t_matrix_col+2);
        ConjugateIndexMap[std::make_pair(s_matrix_row+2, t_matrix_col+2)] = std::make_pair(s_matrix_row, t_matrix_col);
        /// add -5'- -3' inner edge
        s_matrix_row = s_matrix_col = 4*node_id+2;
        t_matrix_row = t_matrix_col = 4*node_id+3;
        ConjugateMatrix[s_matrix_row][t_matrix_col] = v->getWeight()->getCapacity();
    }
}
void Graph::setTransFragment(std::vector<transFrag>& transFrags){
    this->transFrags = transFrags;
}
std::vector<transFrag>&  Graph::getTransFragment() {
    return transFrags;
}

void Graph::updateMaxI(){
    int maxi = 0;
    float maxV = 0.0;
    for (auto v : *this->vertices) {
        if (v->getWeight()->getCoverage() > maxV && !v->getIsGSink() && !v->getIsGSource()){
            maxV = v->getWeight()->getCoverage();
            maxi = v->getIdx();
...
bamStatus* Graph::getBamStatus() {
    return this->bStatus;
}

void Graph::setBamStatus(struct bamStatus* status) {
    this->bStatus = status;
}

bool  Graph::geaphIsLinear(){
    for (auto v : *this->vertices) {
        if (v->getEp3()->getOutEdges()->size()>1){
            return false;
        }
    }
    return true;
