...
Junction::Junction(Vertex *sourceVertex, Vertex *targetVertex, char sourceDir, char targetDir, float copyNum,
    ...
    this->weight->setCapacity(copyNum);
    weight->setCopyNum(copyNum);
    this->source = sourceVertex;
    this->target = targetVertex;
    // what is hasLowerBoundLimit
    this->hasLowerBoundLimit = aIsBounded;
    this->removed = false;
    if (sourceVertex->getIsGSource()||targetVertex->getIsGSink()) {
        this->junctionToEdgeForPesudoVertex();
    }else{
        this->junctionToEdge();
    }
}
...
void Junction::junctionToEdgeForPesudoVertex() {
...
//                               _OUTER_EDGE_);
//    }
    if (this->source->getIsGSource() && this->target->getIsStart()) {
        this->oEdge = new Edge(this->source->getEp3(), this->target->getEp5(), this->weight, _OUTER_EDGE_);
    } else if (this->source->getIsGSource() && this->target->getIsConStart()) {
        this->oEdge = new Edge(this->source->getEp3(), this->target->getRep5(), this->weight,
                               _OUTER_EDGE_);
    } else if (this->source->getIsEnd() && this->target->getIsGSink()) {
        this->oEdge = new Edge(this->source->getEp3(), this->target->getEp5(), this->weight, _OUTER_EDGE_);
    } else if (this->source->getIsConEnd() && this->target->getIsGSink()) {
        this->oEdge = new Edge(this->source->getRep3(), this->target->getEp5(), this->weight,
                               _OUTER_EDGE_);
