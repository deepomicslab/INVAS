...
void parseTransFrag(std::string& f_name, seqGraph::Graph* g) {
    // file format:
//    1 1,2,3,4, 30.5
//    1 1,2,4, 115.0
//    1 1,3,4, 1.0
//    1 1,4, 115.000008
    std::ifstream transf(f_name);
    std::string line;
    std::vector<seqGraph::transFrag> transFrags;
    int transfrg_idx = 0;
    while (std::getline(transf, line)) {
        ...
        // 输出读取的结果（或者进行其他处理）
        std::cout << "Strand: " << strand << ", Indices: ";
        for (int idx : indexVector) {
            std::cout << idx << ' ';
        }
        std::cout << ", Value: " << value << std::endl;
        for (int point : indexVector) {
            g->paths_through_point[point].push_back({indexVector, transfrg_idx});
        }
        transfrg_idx++;
        seqGraph::transFrag tf(strand, indexVector, value);
        transFrags.emplace_back(tf);
    }

    transf.close(); // 关闭文件
    g->setTransFragment(transFrags);
...
//    parse options
    ...
    options.add_options()
            ...
            ("a, rna", "rna mode", cxxopts::value<bool>()->default_value("false"))
            ("t,trans_fragment", "Trans Fragment file", cxxopts::value<std::string>())
            ("e,bam_status", "trans len ,cov in bam", cxxopts::value<std::string>())
    ...
    std::string transF = "";
    std::string bamStatus = "";
    if (result["trans_fragment"].count() > 0){
        transF = result["trans_fragment"].as<std::string>();
    }
    if (result["bam_status"].count() > 0){
        bamStatus = result["bam_status"].as<std::string>();
    }
...
//            TODO, take coverage into consideration
            if (IS_RNA){
                iss>>startTag>>originalSource>>aboundance>>copyNum>>cov>>signal>>conCoverage;
            }else{
...
//                int mm = 9;
//            }
            auto v = decltype(g->addVertex(source,"xx",1,2,aboundance,1,copyNum, 0, cov, signal, conCoverage))();
            if (IS_RNA){
                v = g->addVertex(source,"xx",1,2,aboundance,1,copyNum, 0, cov, signal, conCoverage);
            }else{
                v = g->addVertex(source,"xx",1,2,coverage,1,copyNum, 0);
...
//            score = 1;
            ...
            if (!ignore_copy) {
                for (int i = 1; i < copyNum; i++) {
                    ...
                    if (IS_RNA){
                        v = g->addVertex(source,"xx",1,2,aboundance,1,copyNum, i, cov, signal, conCoverage);
                    }else{
...
//    matching for each connected graph
    ...
    int maxGCount = g->subGraphCount();
//    int maxI = 3;
    std::cout<<"Isolated nodes"<<g->getIsolatedVs()->size()<<std::endl;
    for (auto item : *g->getIsolatedVs()) {
//        resultFile<<item->getOriginId()<<"+"<<"\n";
        std::cout<<item->getOriginId()<<std::endl;
    }
    std::vector<std::string> to_be_remove_sl;
    while (n < 1) {
//        auto subGraph = g->getSubgraph(n);
//        auto subGraph = g;
//        if (subGraph->geaphIsLinear()){
//            std::cout<<"sub graph "<<n<<" is linear\n";
//            for (auto &node: *subGraph->getVertices()){
//                std::cout<<node->getOriginId()<<std::endl;
//            }
//            n++;
//            continue;
//        }
...
//        subGraph->removeByGeneAndScore(cyclePathsFile);
        ...
        // get all 5' start nodes and 3' end nodes (0 edges)
        std::vector<seqGraph::Vertex*> startNodes;
        std::vector<seqGraph::Vertex*> endNodes;
        std::vector<seqGraph::Vertex*> conStartNodes;
        std::vector<seqGraph::Vertex*> conEndNodes;
        for (auto & nd: *subGraph->getVertices()){
            if (nd->getEp5()->getInEdges()->size() == 0 && nd->getRep3()->getOutEdges()->size() == 0 && nd->getEp3()->getOutEdges()->size()==0 && nd->getRep5()->getInEdges()->size() == 0){
                startNodes.emplace_back(nd);
                conEndNodes.emplace_back(nd);
                nd->setIsStart(true);
                nd->setIsConEnd(true);
                conStartNodes.emplace_back(nd);
                endNodes.emplace_back(nd);
                nd->setIsConStart(true);
                nd->setIsEnd(true);
            }
            else if (nd->getEp5()->getInEdges()->size() == 0 && nd->getRep3()->getOutEdges()->size() == 0){
                startNodes.emplace_back(nd);
                conEndNodes.emplace_back(nd);
                nd->setIsStart(true);
                nd->setIsConEnd(true);
            } else if (nd->getEp3()->getOutEdges()->size()==0 && nd->getRep5()->getInEdges()->size() == 0){
                conStartNodes.emplace_back(nd);
                endNodes.emplace_back(nd);
                nd->setIsConStart(true);
                nd->setIsEnd(true);
        ...
        // get all conStart nodes and conEnd nodes (0 edges)


        std::cout<<"start nodes: "<<startNodes.size()<<std::endl;
        std::cout<<"end nodes: "<<endNodes.size()<<std::endl;
        ...
        /// add fake source node and sink node
        if (IS_RNA){
            subGraph->addVertex("s-s_0", "xx", 1,2,0,1,1,0, INF,"",0);
            subGraph->addVertex("t-t_0", "xx",1,2,0,1,1,0, INF,"",0);

        }else{
            subGraph->addVertex("s-s_0", "xx", 1,2,0,1,1,0);
            subGraph->addVertex("t-t_0", "xx",1,2,0,1,1,0);
        }

        // set source node
        std::string source_id ="s-s_0";
        std::string target_id ="t-t_0";
        subGraph->setSource(source_id);
        // set sink node
        subGraph->setSink(target_id);



        // get last node
        auto source_node = subGraph->getSource();
        // get last two nodes
        auto sink_node = subGraph->getSink();


        // add junction for pesudo source node to start nodes
        for (auto & nd: startNodes){
            subGraph->addJunction(source_node, nd, '+', '+', 0, 1, 1);
        }
        for (auto & nd: conStartNodes){
            subGraph->addJunction(source_node, nd, '+', '-', 0, 1, 1);
        }
        for (auto & nd: endNodes){
            subGraph->addJunction(nd, sink_node, '+', '+', 0, 1, 1);
        }
        for (auto & nd: conEndNodes){
            subGraph->addJunction(nd, sink_node, '-', '+', 0, 1, 1);
        }


        auto test =subGraph->getVertices()->front()->getEp5()->getOutEdges();
        auto test2  =subGraph->getVertices()->front()->getEp5()->getInEdges();
        auto test3 =subGraph->getVertices()->front()->getEp3()->getInEdges();

        std::cout<< "source idx:" << source_node->getIdx() << std::endl;
        std::cout<< "sink idx:" << sink_node->getIdx() << std::endl;
        std::cout<<"sub graph nodes: "<<subGraph->getVertices()->size()<<std::endl;
        // print all junction
        std::cout<<"junctions"<<std::endl;
        for (auto &juncs: *subGraph->getJunctions()){
            std::cout<<juncs->getInfo()<<std::endl;
            std::cout<<juncs->getOEdge()->getInfo()<<std::endl;
            std::cout<<"ep3_in: "<< juncs->getSource()->getEp3()->getInEdges()->size()<<std::endl;
            std::cout<<"ep3_out: "<< juncs->getSource()->getEp3()->getOutEdges()->size()<<std::endl;
            std::cout<<"ep5_in: "<< juncs->getTarget()->getEp5()->getInEdges()->size()<<std::endl;
            std::cout<<"ep5_out: "<< juncs->getTarget()->getEp5()->getOutEdges()->size()<<std::endl;
        }


        for (auto & nd: *subGraph->getVertices()){
            std::cout<<"node\t"<<nd->getStart()<< "\t"<<nd->getEnd()<<"\t"<<nd->getWeight()->getCoverage()<<"\t"\
            <<nd->getIdx()<<"\t"<<nd->getId()<<"\t"<<nd->getEp5()->getInEdges()->size()<<"\t"<<nd->getRep5()->getInEdges()->size()<<"\t"<<\
            nd->getEp5()->getOutEdges()->size()<<"\t"<<nd->getRep5()->getOutEdges()->size()<<"\t"<<std::endl;
            std::cout<<nd->getEp5()->getEdges()->size()<<"\t"<<nd->getRep5()->getEdges()->size()<<std::endl;
        }
...
//        break;
        if (IS_RNA){
            parseTransFrag(transF, subGraph);
        }


        std::cout<<"total nodes"<<g->getVertices()->size()<<std::endl;
//        int maxI = subGraph->subGraphCount();
//    int maxI = 3;
        std::cout<<"Isolated nodes"<<subGraph->getIsolatedVs()->size()<<std::endl;
        auto* mf = new maxFlow(subGraph);
        mf->iterate();
        /// write predicted transcripts
        for (int trans_i; trans_i< mf->preTrans.size(); trans_i++){
            resultFile << "preTrans["<< trans_i+1<<"] :";
            for (auto &p: mf->preTransPath[trans_i]){
                resultFile << mf->GetmatrixIdx2nodeIdxStr()[p]<<"\t";
            }
            resultFile << "\n";
            for (int exon_i=0; exon_i< mf->preTrans[trans_i].size(); exon_i++){
                resultFile << exon_i <<"\t"<<mf->preTrans[trans_i][exon_i].start<<"\t"<<mf->preTrans[trans_i][exon_i].end \
                << "\t" << mf->preTrans[trans_i][exon_i].cov<<std::endl;
            }
