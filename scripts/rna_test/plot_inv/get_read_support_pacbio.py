...
def read_vcf_file(vcf_file):
    ...
    for record in vcf:
        if record.info['SVTYPE'] == 'INV':
            ...
            for rname in record.info['RNAMES']:
                inv_rnames.append(rname)

    return inv_regions, inv_rnames

def read_vcf_file_ol(vcf_file):
    """使用pysam读取VCF文件并提取INV区域"""
    # get all inv reads name in RNAMES tag
    inv_regions = []
    inv_rnames=[]
...
def get_all_reads(bam, inv_regions, ext=10):
    ...
    for read in bam.fetch(chrom, start, end):
        ...
        for clip_start, clip_end in res:
            ...
            if do_intervals_overlap((clip_start, clip_end), (start-ext, end-ext)):
                is_support = True
                break
        if is_support:
...
def assign_reads_in_invs(subset, inv_regions, inv_rnames):
    ...
    if len(inv_regions) ==0:
        ...
        all_supp_reads_names = inv_rnames
        # all_supp_reads_names = get_all_reads(bam, inv_regions)
        # print("all reads number:", len(all_reads), len(set(all_reads)))
    print("support reads number:", len(all_supp_reads_names), len(set(all_supp_reads_names)))
    for chr, start, end in inv_regions:
        # get allreads for each reads
        for i in range(start, end+1):
            # loop all aligned reads in this position
            for read in bam.fetch(chr, i, i+1):
...
def assign_reads_in_invs_ol(subset, inv_regions, inv_rnames):
    # get all reads from bam file for inv regions
    bam = pysam.AlignmentFile(args.bam_file, "rb")
    # if len(inv_regions) ==0:
    #     all_supp_reads_names = get_all_reads(bam, inv_regions)
...
def main(depth_file, vcf_file):
    ...
    # print(depth_df.head())
    inv_regions, inv_rnames = read_vcf_file(vcf_file)
    inv_regions, inv_rnames = read_vcf_file_ol(vcf_file)
    print("inv:",inv_rnames)
    print("invlength:", len(set(inv_rnames)))
    print(inv_regions)
    if len(inv_regions) == 0:
        ...
        # assign_reads_in_invs(subset1, inv_regions, inv_rnames)
        assign_reads_in_invs_ol(subset1, inv_regions, inv_rnames)
        print(subset0)
        print(subset1)
        print(subset2)
