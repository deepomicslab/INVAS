...
def get_mapped_gene_sequence(read):
    """
    根据pysam AlignmentFile read对象，返回它映射到的fasta序列。
    
    Args:
    read: pysam AlignedSegment 对象，代表一个单独的read。
    fasta_file_path: 包含fasta序列的文件路径。
    
    Returns:
    str: 映射到的fasta序列，如果没有找到匹配，则为None。
    """
    # 确保read是已映射的
    if read.is_unmapped:
        return None
    ...
    start = read.reference_start  # read映射的开始位置
    end = read.reference_end      # read映射的结束位置
    
    # 打开fasta文件
    fasta = exon_db
    
    # 遍历fasta中的每个序列，寻找匹配的序列
    for seq in fasta.keys():
        # fasta中序列的名称格式为 'chr:start-end'
        seq_name, seq_range = seq.split(':')
        seq_start, seq_end = map(int, seq_range.split('-'))
        
        # 检查read的参考序列和位置是否匹配fasta中的序列
        print(seq_name, seq_start, seq_end, ref_name, start, end)
        if seq_name == ref_name and seq_start <= start and seq_end >= end:
            # 获取并返回匹配的fasta序列
            return str(fasta[seq].seq), seq
    
    # 如果没有找到匹配的序列，返回None
    return None, None

def process_bwa2(reads_dict1):
    for rname, rec in reads_dict1.items():
        for idx, r in enumerate(rec):
            if r.query_name not in ["UNC11-SN627:379:C5FDFACXX:3:1315:3565:79425_sg"]:
                continue
            if 'S' in r.cigarstring and not r.is_supplementary:
                r_cp = copy_read(r)
                print("##########################################################################################")
                ...
                if left_soft_clip > 2 and right_soft_clip <=2:
                    ...
                    left_soft_seq = get_query_seq_for_inv_rg(r, left_soft_clip, "l")
                    # get right exon sequence
                    gene_seq, seq_name = get_mapped_gene_sequence(r)
                    print(seq_name, type(gene_seq))
                    if not gene_seq or not seq_name:
                        continue
                    # if right_exon_seq == "" or cur_header=="":
                    #     continue
                    # print("left_soft_seq:", left_soft_seq)
                    # print("right_exon_seq:", right_exon_seq)
                    align_res = best_alignment2(left_soft_seq, gene_seq)
                    print("test_align res:",align_res['ref_begin'], align_res['ref_end'])
                    if align_res:
                        match_len=align_res['read_end']-align_res['read_begin']+1
                        still_soft_len=len(left_soft_seq)-align_res['read_end']
                    ...
                    # print("left_soft_seq:", left_soft_seq)
                    # print("right_exon_seq:", right_exon_seq)
                    align_res = best_alignment2(right_soft_seq, gene_seq)
...
def reformat_bam(reads_dict1, reads_dict2):
    ...
    reads_dict1 = process_bwa2(reads_dict1)
...
if __name__ == "__main__":
    ...
    outf=pysam.AlignmentFile(out_bam, 'wb', header=reads_dict1[list(reads_dict1.keys())[0]][0].header)
    # r1_dict = fastq_to_dict(fq1)
    # r2_dict = fastq_to_dict(fq2)
    print("reading unmap fq")
