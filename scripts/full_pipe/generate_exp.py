...
def process_trans():
    ...
    trans_index = 1
    for trans_id, exons_dict in transcripts_dict.items():
        res_f.write(f"""{args.chrom}\tSeqflow\ttranscript\t{trans_range_dict[trans_id][0]}\t{trans_range_dict[trans_id][1]}\t1000\t+\t.\tgene_id "SEQFLOW.1"; transcript_id "SEQFLOW1.{trans_index}"; cov "{trans_dict[trans_id]}"; FPKM "{trans_exp_dict[trans_id]['fpkm']}" TPM "{trans_exp_dict[trans_id]['tpm']}"\n""")
        ...
        for exon_id, (start, end, cov) in exons_dict.items():
            ...
            res_f.write(f"""{args.chrom}\tSeqflow\texon\t{start}\t{end}\t1000\t{exon_strands[exon_idx]}\t.\tgene_id "SEQFLOW.1"; transcript_id "SEQFLOW1.{trans_index}"; exon_number "{exon_id+1}" cov "{cov}"\n""")
            exon_idx += 1
        trans_index +=1
