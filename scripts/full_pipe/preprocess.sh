...
python $script_dir/invvcf2bed.py "$inv_vcf" "$inv_bed" 10000

for chr in {1..22} X; do
    echo Processing $chr
    # example: awk '$1 == "12"' hg38_genes.bed > chr12_genes.bed
    awk -v chr=$chr '$1 == chr' "$hg38_genes_bed" > "$outdir"/chr${chr}_genes.bed
    awk 'BEGIN {OFS="\t"} {$1="chr"$1; print}' "$outdir"/chr${chr}_genes.bed > "$outdir"/temp.bed && mv "$outdir"/temp.bed "$outdir"/chr${chr}_genes.bed
    gene_bed="$outdir"/chr${chr}_genes.bed
    # extract inversions on this chromosome

    awk -v chr="chr$chr" '$1 == chr' "$inv_bed" > "$outdir"/chr${chr}_inv.bed
    ...
    python $script_dir/find_invs_withingenes2.py "$outdir"/chr${chr}_inv.bed "$outdir"/chr${chr}_genes.bed "$outdir"/chr${chr}_inv_in_gene.bed
    # python inversion_genes_mapping2.py inv_gene_map.bed inv_gene_map.txt
    python $script_dir/inversion_genes_mapping2.py "$outdir"/chr${chr}_inv_in_gene.bed "$outdir"/chr${chr}_inv_in_gene.txt
    # python filter_dup_inv.py inv_gene_map.txt inv_gene_map_dedup.txt
    python $script_dir/filter_dup_inv.py "$outdir"/chr${chr}_inv_in_gene.txt "$outdir"/chr${chr}_inv_in_gene_dedup.txt
    # samtools view still_unmap_bwa.s.bam chr12 -O BAM -o still_unmap_bwa.s.chr12.bam
    samtools view "$still_unmap_bwa_bam" chr${chr} -O BAM -o "$outdir"/still_unmap_bwa.s.chr${chr}.bam
    samtools index "$outdir"/still_unmap_bwa.s.chr${chr}.bam
    ...
    # python get_high_conf_rg.py still_unmap_bwa.s.chr12.bam chr12 5 --gap 0 > high_conf.2.bed
    python $script_dir/get_high_conf_rg2.py "$outdir"/still_unmap_bwa.s.chr${chr}.bam chr${chr} 5 --gap 0 > "$outdir"/high_conf.${chr}.bed
    ...
    python $script_dir/filter_conf_inv2.py "$outdir"/high_conf.${chr}.bed "$outdir"/chr${chr}_inv_in_gene_dedup.txt "$outdir"/chr${chr}_inv_in_gene_dedup_highcof.txt

    # get gene region bam for each region in chr${chr}_inv_in_gene_dedup_highcof.txt
    region_file="$outdir"/chr${chr}_inv_in_gene_dedup_highcof.txt
