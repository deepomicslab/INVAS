    ...
    with open(gtf, 'r') as file:
        for line_id, line in enumerate(file):
            ...
            if line[2] == 'exon':
                ...
                print(f"processing exon: ({start}, {end}), {inv_region}, {overlap}", "visited_invs:",visited_invs)
                if overlap:
                    if visited_invs[-1][0]>= start and visited_invs[-1][1]>=end:
                        print("type1")
                        exon1_start=start
                        exon1_end=visited_invs[-1][0]
                        exon2_start=overlap[0]
                        exon2_end=overlap[1]
                        exon3_start=end
                        exon3_end=visited_invs[-1][1]
                    elif visited_invs[-1][0]>= start and visited_invs[-1][1]<=end:
                        print("type2")
                        exon1_start=start
                        exon1_end=overlap[0]
                        exon2_start=overlap[0]
                        ...
                        print("type3")
                        exon1_start=visited_invs[-1][0]
                        exon1_end=overlap[0]
                        exon2_start=overlap[0]
                        exon2_end=overlap[1]
                        exon3_start=visited_invs[-1][1]
                        exon3_end=end
                    else:
...
def main():
    ...
    print("normal_exons_tuples:", normal_exons_tuples)
    print("inv_exon_cov_dict:", inv_exon_cov_dict)
    normal_exons_tuples=list(set(normal_exons_tuples))
    # sort exon_cov_dict key by start position
    exon_cov_dict = dict(sorted(exon_cov_dict.items(), key=lambda item: item[0][0]))
    if len(normal_exons_tuples) <= 1:
        print("no enough exons")
        return
    ...
    #     print(f"{seg}")
    print("len segs:", len(normal_exons_tuples), len(exon_cov_dict))
    ...
    with open(args.segment_o_f, 'w') as file:
        ...
        for exon, cov in exon_cov_dict.items():
            exon_s_e=[exon[0], exon[1]]
            exon_s_e.sort()

            file.write(f"{exon_s_e[0]}-{exon_s_e[1]}\t{cov}\n")
        file.write(f"0-0\t{0.0000}\n")
    ...
    with open(args.trf_o_f, 'w') as file:
        ...
        for i, trans in enumerate(trans_fregments):
            ...
            # print(['1',trans_f_order_tmp, trf_covs[i]])
