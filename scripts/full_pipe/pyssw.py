...
current_script_dir=op.dirname(op.realpath(__file__))
sLibPath = current_script_dir
...
def align(debug_q, debug_t, is_r=False):


    # sLibPath = "/run/media/wangxuedong/5C40783348454630/cityu/software/seqFlow/scripts/test_inv/libssw.so"
    sLibPath = current_script_dir
    nMatch = 2
    nMismatch = 2
    nOpen = 3
    ...
    debug_q = [("test_q", debug_q, "")]
    debug_t = [("test_t", debug_t, "")]
    if is_r:
        bBest=True                   
    lEle = []
    dRc = {} 
...
# init DNA score matrix
        if not sMatrix:
            ...
            lEle, dEle2Int, dInt2Ele, lScore = ssw_lib.read_matrix(sMatrix)
    else:
...
# set flag
    ...
    if bPath:
        nFlag = 2
# print sam head
    if bSam and bHeader and bPath:
        print('@HD\tVN:1.4\tSO:queryname')
        for sRId,sRSeq,_ in debug_t:
            print('@SQ\tSN:{}\tLN:{}'.format(sRId, len(sRSeq)))
    elif bSam and not bPath:
...
# print results
    ...
    # return score ref_begin ref_end query_begin query_end cigar
    res = {
        "score": resPrint[0],
        "ref_begin": resPrint[2] + 1,
        "ref_end": resPrint[3] + 1,
        "read_begin": resPrint[4] + 1,
        "read_end": resPrint[5] + 1,
        "cigar": sCigar
    }
    return res
