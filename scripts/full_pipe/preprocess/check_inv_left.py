            ...
            if 'S' in r.cigarstring and not r.is_supplementary:
                ...
                if left_soft_clip > 2 and right_soft_clip <=2:
                    ...
                    if align_res:
                        ...
                        new_match_end = real_inv_region[2] - (r.reference_start-real_inv_region[1])
...
def check_bam():
    ...
    bam_f = pysam.AlignmentFile(args.bam, "rb")
    for r in bam_f:
        if 'S' in r.cigarstring and not r.is_supplementary:
            r_cp = copy_read(r)
            # print("before:", r)
            # print("##########################################################################################")
            left_soft_clip, left_tag, right_soft_clip, right_tag = parse_cigar2(r.cigarstring)
            if right_soft_clip > 15:
                print("right_soft_clip:", r.query_sequence[-right_soft_clip:][::-1])
                best_alignment2(r.query_sequence[-right_soft_clip:][::-1], ref_seq)
